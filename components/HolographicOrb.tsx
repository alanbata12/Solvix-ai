...
const services = [
  { icon: TrendingUp, title: 'Forex Control Room', desc: 'Broker-connected workflows with explicit risk gates and human control.', color: '#7c6cff', glow: 'rgba(124,108,255,.22)', stat: 'RISK GATED' },
  ...
  { icon: Cpu, title: 'AI Agent Fleet', desc: 'Specialized agents for discovery, execution support, repair and monitoring.', color: '#8b7cff', glow: 'rgba(139,124,255,.20)', stat: 'CONTROLLED' },
  { icon: Wallet, title: 'Verified Wallet', desc: 'Separate projected, pending and verified money with auditable records.', color: '#34d399', glow: 'rgba(52,211,153,.20)', stat: 'AUDIT READY' },
...
export default function HolographicOrb() {
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden flex items-center justify-center" style={{ perspective: '1200px' }}
      ...
      <div className="absolute w-[340px] h-[340px] rounded-full blur-[90px] opacity-30" style={{ background: 'radial-gradient(circle,rgba(0,240,255,.35),transparent 70%)', transform: 'translate(' + mouse.x * .5 + 'px,' + mouse.y * .5 + 'px)' }} />
      <div className="relative" style={{ transformStyle: 'preserve-3d', transform: 'rotateX(' + -mouse.y * .3 + 'deg) rotateY(' + mouse.x * .3 + 'deg)', animation: 'solvix-float 6s ease-in-out infinite' }}>
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
          <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border border-cyan-400/30 flex items-center justify-center" style={{ animation: paused ? 'none' : 'solvix-spin 20s linear infinite', boxShadow: '0 0 60px rgba(0,240,255,.2),inset 0 0 60px rgba(0,240,255,.1)' }}>
            <div className="w-24 h-24 md:w-28 md:h-28 rounded-full border border-purple-400/40 flex items-center justify-center">
              <div className="w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center" style={{ background: 'radial-gradient(circle,rgba(0,240,255,.8),rgba(168,85,247,.6))', boxShadow: '0 0 80px rgba(0,240,255,.5),0 0 120px rgba(168,85,247,.3)', animation: 'solvix-pulse 3s ease-in-out infinite' }}><span className="text-2xl md:text-3xl font-black text-black font-mono">S</span></div>
            </div>
          </div>
          <div className="absolute inset-[-55px] rounded-full border border-cyan-400/10" style={{ transform: 'rotateX(75deg)', animation: paused ? 'none' : 'solvix-spin 30s linear infinite' }} />
          <div className="absolute inset-[-80px] rounded-full border border-purple-400/10" style={{ transform: 'rotateX(75deg) rotateZ(60deg)', animation: paused ? 'none' : 'solvix-reverse 25s linear infinite' }} />
        </div>
