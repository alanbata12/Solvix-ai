...
export default async function GlobalDashboard() {
  ...
  const [{ data: profile }, { data: wallets }, { data: agents }, { data: leads }, { data: heartbeats }, { data: gate }] = await Promise.all([
    ...
    supabase.from('job_leads').select('id,title,client_region,expected_payout_usd,lead_price_usd,difficulty,required_skills,is_verified,source_name,source_url').eq('is_verified', true).order('created_at', { ascending: false }).limit(6),
    supabase.from('agent_heartbeats').select('agent,status,last_success_at,updated_at,runs,successes,failures,opportunities_found').order('updated_at', { ascending: false }),
  ...
  const now = Date.now();
  const liveHeartbeats = (heartbeats || []).filter(h => h.updated_at && now - new Date(h.updated_at).getTime() < 15 * 60 * 1000);
  ...
  return <main className="min-h-screen bg-[#050508] text-white p-4 md:p-6 font-sans">
    <div className="max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3 mb-8 pb-4 border-b border-white/5">
        ...
        <div className="flex gap-4 text-xs font-mono text-gray-500"><span>JOB LEADS: <span className="text-green-400">{leads?.length ?? 0} LIVE</span></span><span>TRADING: <span className={gate?.live_enabled ? "text-green-400" : "text-red-400"}>{gate?.live_enabled ? "ENABLED" : "LOCKED"}</span></span></div>
      </div>
      <div className="grid grid-cols-12 gap-5">
        ...
        <section className="col-span-12 md:col-span-5 bg-white/5 border border-white/10 rounded-3xl p-6">
          <div className="flex items-center gap-2 mb-4"><Activity className="w-5 h-5 text-cyan-400"/><span className="text-xs font-mono text-gray-400 tracking-widest">EXECUTION CONTROL</span></div>
        ...
        <section className="col-span-12 md:col-span-3 bg-white/5 border border-white/10 rounded-3xl p-6">
          <div className="flex items-center gap-2 mb-4"><Zap className="w-5 h-5 text-green-400"/><span className="text-xs font-mono text-gray-400 tracking-widest">LIVE AGENT STATE</span></div>
          <div className="space-y-3">
            {liveHeartbeats.length ? liveHeartbeats.slice(0,8).map(a=><div key={a.agent} className="p-3 bg-black/40 rounded-xl border border-white/5">
              <div className="flex justify-between gap-2"><p className="text-sm font-bold truncate">{a.agent}</p><span className="text-[10px] text-green-400">LIVE</span></div>
              <div className="flex justify-between mt-2 text-[10px] font-mono"><span className="text-gray-400">{a.status}</span><span className="text-gray-500">{a.successes}/{a.runs} success</span></div>
            </div>) : <p className="text-gray-500 text-sm">No recent agent heartbeat.</p>}
          </div>
        </section>

        <section className="col-span-12 md:col-span-8 bg-white/5 border border-white/10 rounded-3xl p-6">
          <div className="flex items-center justify-between gap-3 mb-4"><div className="flex items-center gap-2"><Zap className="w-5 h-5 text-yellow-400"/><span className="text-xs font-mono text-gray-400 tracking-widest">GLOBAL JOB LEADS</span></div><span className="text-xs font-mono text-green-400">{leads?.length ?? 0} SOURCED</span></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{(leads||[]).length ? leads!.map(l=><a key={l.id} href={l.source_url || '/jobs'} target="_blank" rel="noreferrer" className="block p-4 bg-black/40 rounded-xl border border-white/5 hover:border-white/20"><div className="flex justify-between gap-3 mb-2"><span className="text-[10px] font-mono text-gray-500">{l.client_region||'GLOBAL'}</span><span className="text-[10px] font-mono text-green-400">SOURCE LINKED</span></div><h3 className="font-bold mb-1">{l.title}</h3><p className="text-xs text-gray-500">{l.source_name||'External source'}</p><div className="flex justify-between text-xs text-gray-400 mt-3"><span>Expected: <span className="text-white font-mono">{l.expected_payout_usd == null ? 'Not stated' : '$'+Number(l.expected_payout_usd).toLocaleString()}</span></span><span>{l.difficulty||'Unspecified'}</span></div></a>) : <p className="text-sm text-gray-500">No sourced job leads are currently stored.</p>}</div>
        </section>

        <section className="col-span-12 md:col-span-4 bg-gradient-to-br from-white/5 to-transparent border border-white/10 rounded-3xl p-6">
          <div className="flex items-center gap-2 mb-4"><Shield className="w-5 h-5 text-cyan-400"/><span className="text-xs font-mono text-gray-400 tracking-widest">ZERO-TRUST VAULT</span></div>
          <div className="space-y-4 text-sm"><div className="flex justify-between"><span className="text-gray-400">Settlement</span><span className="font-mono text-white">Provider-gated</span></div><div className="flex justify-between"><span className="text-gray-400">Ledger</span><span className="font-mono text-green-400">Auditable</span></div><div className="flex justify-between"><span className="text-gray-400">Trading gate</span><span className="font-mono text-red-400">{gate?.live_enabled ? 'OPEN' : 'LOCKED'}</span></div><p className="text-xs text-gray-600">Payments remain separate from job discovery and execution. External payout verification is required before revenue is treated as verified.</p></div>
        </section>
      </div>
    </div>
  </main>;
