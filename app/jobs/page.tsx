...
export default function JobsPage() {
  ...
  return <main className="min-h-screen bg-[#050508] text-white p-4 md:p-8">
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <p className="text-xs font-mono tracking-widest text-violet-400">SOLVIX OPPORTUNITY MARKET</p>
        <h1 className="text-3xl md:text-5xl font-bold mt-2">Jobs & Paychecks</h1>
        <p className="text-gray-400 mt-3">Review available work and its listed reward first. Billing appears only after you select an opportunity.</p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {jobs.map(job => <button key={job.id} onClick={() => setSelected(job)} className="text-left p-5 bg-white/5 border border-white/10 rounded-2xl hover:border-violet-400/50">
          <div className="flex justify-between gap-3"><span className="text-xs text-gray-500">GLOBAL</span><span className="text-xs text-green-400">DISCOVERED</span></div>
          ...
          <div className="mt-5 flex justify-between">
            <span className="text-gray-400">Listed reward</span>
            <strong className="text-violet-300">{job.reward == null ? "Not stated" : Number(job.reward).toLocaleString() + " " + (job.currency || "")}</strong>
          </div>
          <p className="text-xs text-gray-500 mt-2">Select to view details and billing</p>
        </button>)}
        {!jobs.length && <p className="text-gray-500">No active opportunities are currently available.</p>}
      </div>}
      ...
      {selected && <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm p-4 flex items-center justify-center" onClick={() => setSelected(null)}>
        <section className="w-full max-w-lg bg-[#0b0b12] border border-violet-400/30 rounded-3xl p-6" onClick={e => e.stopPropagation()}>
          <p className="text-xs font-mono text-violet-400">OPPORTUNITY SELECTED</p>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            ...
            <div className="p-4 bg-black/40 rounded-xl"><p className="text-xs text-gray-500">ACCESS / BILLING</p><p className="text-xl font-bold text-violet-300">"Calculated at checkout"</p></div>
          </div>
          <p className="text-sm text-gray-400 mt-5">You are not charged by opening or reviewing this opportunity. Continue only when you are ready to confirm the billing.</p>
