  ...
  return <main className="min-h-screen bg-[#030305] text-white overflow-x-hidden">
    ...
    <section id="targets" className="py-20 px-4 border-t border-gray-800/50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10"><div className="inline-flex items-center gap-2 text-violet-400 font-mono text-xs tracking-widest mb-3"><Target className="w-4 h-4"/> X100 PRO MARKET STRATEGY</div><h2 className="text-3xl md:text-5xl font-bold mb-3">The numbers we are <span className="text-violet-400">building toward.</span></h2><p className="text-gray-400 max-w-2xl mx-auto">These are strategic targets and positioning figures—not current user counts, verified payouts, measured win rates, or guaranteed returns.</p></div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {marketTargets.map(([value,label,note])=><div key={label} className="bg-gray-900/50 border border-gray-800 rounded-2xl p-5"><p className="text-3xl md:text-4xl font-black font-mono text-violet-400">{value}</p><p className="font-bold mt-2">{label}</p><p className="text-[11px] text-gray-600 mt-2">{note}</p></div>)}
        </div>
      </div>
    </section>
