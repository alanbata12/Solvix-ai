  ...
  return <main className="min-h-screen bg-[#030305] text-white overflow-x-hidden">
    ...
    <section id="services" className="relative pt-28">
      <div className="text-center px-4 relative z-20">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center px-4"><Link href="/login" className="px-8 py-4 bg-cyan-500 text-black font-bold rounded-xl flex items-center justify-center gap-2">START ONBOARDING <ArrowRight className="w-5 h-5"/></Link><a href="#targets" className="px-8 py-4 bg-gray-900/50 border border-gray-800 rounded-xl font-bold">VIEW MARKET STRATEGY</a></div>
    ...
    <section id="targets" className="py-20 px-4 border-t border-gray-800/50">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10"><div className="inline-flex items-center gap-2 text-cyan-400 font-mono text-xs tracking-widest mb-3"><Target className="w-4 h-4"/> X100 PRO MARKET STRATEGY</div><h2 className="text-3xl md:text-5xl font-bold mb-3">The numbers we are <span className="text-cyan-400">building toward.</span></h2><p className="text-gray-400 max-w-2xl mx-auto">These are strategic targets and positioning figures—not current user counts, verified payouts, measured win rates, or guaranteed returns.</p></div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {marketTargets.map(([value,label,note])=><div key={label} className="bg-gray-900/50 border border-gray-800 rounded-2xl p-5"><p className="text-3xl md:text-4xl font-black font-mono text-cyan-400">{value}</p><p className="font-bold mt-2">{label}</p><p className="text-[11px] text-gray-600 mt-2">{note}</p></div>)}
        </div>
      </div>
    </section>
    ...
    <section className="py-20 px-4 border-t border-gray-800">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10"><h2 className="text-3xl md:text-5xl font-bold">24/7 Agent <span className="text-purple-400">Fleet Vision</span></h2><p className="text-gray-400 mt-3">Proposed commercial positioning. Performance figures remain unverified until backed by audited live results.</p></div>
        <div className="grid md:grid-cols-4 gap-4">
          {agentTargets.map(([name,session,win,fee])=><div key={name} className="bg-gray-900/50 border border-gray-800 rounded-2xl p-5"><div className="flex items-center justify-between"><TrendingUp className="w-5 h-5 text-purple-400"/><span className="text-[10px] text-gray-600 font-mono">TARGET</span></div><h3 className="text-xl font-bold mt-4">{name}</h3><p className="text-xs text-gray-500 font-mono">{session}</p><div className="mt-5 flex justify-between"><span className="text-green-400 font-mono">{win}</span><span className="text-gray-300 font-mono">{fee}</span></div><p className="text-[10px] text-gray-600 mt-2">Strategy benchmark / proposed pricing</p></div>)}
        </div>
      </div>
    </section>
    ...
    <section id="trust" className="py-20 px-4 border-t border-gray-800"><div className="max-w-6xl mx-auto"><div className="text-center mb-12"><h2 className="text-3xl md:text-5xl font-bold mb-4">Built for <span className="text-cyan-400">global control.</span></h2><p className="text-gray-400 text-lg">Multi-currency accounting, auditable opportunities and explicit execution gates.</p></div>
      <div className="grid md:grid-cols-3 gap-6">
        {[['Multi-Currency Ledger','Hold supported currencies independently instead of pretending every balance is UGX.',Wallet],['Settlement Abstraction','Payment rails can be connected independently; credentials and provider verification stay server-side.',Globe],['Human Control','Trading remains locked until the backend confirms the required execution conditions.',Shield]].map(([title,desc,Icon])=><div key={title as string} className="bg-gray-900/40 border border-gray-800 rounded-xl p-6"><div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4 bg-cyan-500/10 text-cyan-400"><Icon className="w-6 h-6"/></div><h3 className="text-xl font-bold mb-2">{title as string}</h3><p className="text-gray-400 text-sm leading-relaxed">{desc as string}</p></div>)}
      </div>
    </div></section>

    <section className="py-20 px-4 border-t border-gray-800"><div className="max-w-4xl mx-auto text-center"><Users className="w-8 h-8 text-cyan-400 mx-auto mb-4"/><h2 className="text-3xl md:text-5xl font-bold mb-4">Build the <span className="text-cyan-400">global terminal.</span></h2><p className="text-gray-400 text-lg mb-8">The market strategy is ambitious. The ledger must remain factual. We will promote each target to a verified metric only when the backend has evidence.</p><Link href="/login" className="inline-flex items-center gap-2 px-8 py-4 bg-cyan-500 text-black font-bold rounded-xl">BEGIN SECURE ONBOARDING <ArrowRight className="w-5 h-5"/></Link></div></section>
