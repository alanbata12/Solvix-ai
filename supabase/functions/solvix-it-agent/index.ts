...
Deno.serve(async () => {
  ...
  const blocked = events.filter(
    (e) => e.action === "BLOCKED" || e.action === "TEMPORARY_BLOCK"
  );
  ...
  if (highRisk.length >= 5) {
    ...
    recommendation = "Review affected sources and increase monitoring.";
  }

  if (blocked.length >= 3) {
    health = "ATTENTION_REQUIRED";
    diagnosis = "Multiple temporary restrictions detected.";
    recommendation = "Investigate recurring sources before increasing restrictions.";
  }
